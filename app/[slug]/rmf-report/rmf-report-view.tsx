...
import { FileDown, ArrowLeft, Gauge } from "lucide-react";
      ...
      <div className="mx-auto max-w-[980px] space-y-4 p-6">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">{phoenixProject} · 옵션을 설정하고 보고서를 생성하세요</p>
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="rounded-lg border bg-card p-4">
                <p className="text-xs text-muted-foreground">종합 위험등급</p>
                <p className="mt-1 text-2xl font-bold" style={{ color: gradeColor(score.grade) }}>{score.grade}위험</p>
              </div>
              <div className="rounded-lg border bg-card p-4">
                <p className="text-xs text-muted-foreground">잔여위험 총점</p>
                <p className="mt-1 text-2xl font-bold">{score.total}<span className="text-sm text-muted-foreground"> / 100</span></p>
              </div>
              <div className="rounded-lg border bg-card p-4">
                <p className="text-xs text-muted-foreground">분석 트레이스</p>
                <p className="mt-1 text-2xl font-bold">{trees.length}</p>
              </div>
              <div className="rounded-lg border bg-card p-4">
                <p className="text-xs text-muted-foreground">지적 사항</p>
                <p className="mt-1 text-2xl font-bold">{findings.length}<span className="text-sm text-muted-foreground">건</span></p>
              </div>
            </div>
                ...
                {RISK_SECTIONS.map((sec) => {
                  ...
                  return (
                    <div key={sec.key} className="flex items-center gap-3 text-xs">
                      ...
                      <div className="w-28 shrink-0 text-right"><span className="font-semibold" style={{ color }}>{ratioLabel(ratio)}</span><span className="text-muted-foreground"> · {score.sectionSubtotals[sec.key] ?? 0}/{sec.weight} · 지적 {fcount}</span></div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="rounded-lg border bg-card p-4">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-xs">
                <label className="flex items-center justify-between gap-2">
                  <span className="text-muted-foreground">평가 기간</span>
                  <DateRangePicker value={dateRange} onChange={setDateRange} />
                </label>
                <label className="flex items-center justify-between gap-2">
                  <span className="text-muted-foreground">지적사항 항목당 표시</span>
                  <input type="number" min={1} max={50} value={findingsCap} onChange={(e) => setFindingsCap(Math.max(1, Math.min(50, Number(e.target.value) || 1)))} className="w-20 rounded border px-2 py-1" />
                </label>
                <label className="flex items-center justify-between gap-2">
                  <span className="text-muted-foreground">기관/제출처</span>
                  <input value={orgName} onChange={(e) => setOrgName(e.target.value)} placeholder="예: 금융감독원" className="w-40 rounded border px-2 py-1" />
                </label>
                <label className="flex items-center justify-between gap-2">
                  <span className="text-muted-foreground">평가자</span>
                  <input value={assessor} onChange={(e) => setAssessor(e.target.value)} placeholder="작성자명" className="w-40 rounded border px-2 py-1" />
                </label>
              </div>
              <div className="mt-3 border-t pt-3">
                ...
                <div className="flex flex-wrap gap-3 text-xs">
                  {SECTION_LABELS.map((s) => (
                    <label key={s.key} className="flex cursor-pointer items-center gap-1.5">
                      <input type="checkbox" checked={sections[s.key]} onChange={(e) => setSections((prev) => ({ ...prev, [s.key]: e.target.checked }))} className="rounded" />
                      {s.label}
                    </label>
                  ))}
                </div>
  ...
  return (
    <div className="mx-auto max-w-[880px] p-6">
      ...
      <div className="no-print mb-4 flex items-center justify-between gap-3">
        <button onClick={() => setMode("config")} className="flex items-center gap-1.5 rounded-md border px-3 py-2 text-xs font-medium transition hover:bg-muted">
          <ArrowLeft className="h-4 w-4" /> 설정 수정
        </button>
        <button onClick={() => window.print()} className="flex items-center gap-1.5 rounded-md bg-foreground px-4 py-2 text-sm font-medium text-background transition hover:bg-foreground/80">
